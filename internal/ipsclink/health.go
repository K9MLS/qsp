...
func (h HealthCheck) Check(context.Context) health.Result {
	...
	if ignored > 0 {
		return health.Degraded(
			summary+fmt.Sprintf(", %d from radio IDs not on the allow list", ignored),
			"add the radio ID under ipsc.allowed_peers, or remove the entry that is not meant to connect")
	}
