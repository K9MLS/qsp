...
func TestTheConnectorsConfigurationIsChecked(t *testing.T) {
	good := `{"logon_socket":"/run/qsp/zello.sock","usrp_listen":"127.0.0.1:32002","usrp_peer":"127.0.0.1:32001","health_listen":"127.0.0.1:18090"}`
	tests := []struct {
		name, body, want string
	}{
		{"valid", good, ""},
		{"a misspelled field is refused", strings.Replace(good, `"usrp_peer"`, `"usrp_pear"`, 1), "unknown field"},
		{"the channel is QSP's now, not this file's", strings.Replace(good, `"usrp_peer"`, `"channel":"K9MLS","usrp_peer"`, 1), "unknown field"},
		...
		{"a plain-text endpoint", strings.Replace(good, `"usrp_peer"`, `"endpoint":"ws://zello.io/ws","usrp_peer"`, 1), "wss://"},
		{"a password has nowhere to go", strings.Replace(good, `"usrp_peer"`, `"password":"x","usrp_peer"`, 1), "unknown field"},
	}
